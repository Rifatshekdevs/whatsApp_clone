...
class SoundRecordNotifier extends ChangeNotifier {
  ...
  /// used to update space when drag the button to left
  late double edge;
  ...
  late CommonController commonController = Get.find<CommonController>();
  ...
  resetEdgePadding() async {
    ...
    edge = 0;
    buttonPressed = false;
    commonController.isShow.value = buttonPressed;
    second = 0;
    minute = 0;
    isShow = false;
    heightPosition = 0;
  ...
  Future<String> getFilePath() async {
    ...
    if (Platform.isIOS) {
      ...
      _sdPath = initialStorePathRecord.length == 0 ? "/storage/emulated/0/new_record_sound" : initialStorePathRecord;
    }
  ...
  updateScrollValue(Offset currentValue, BuildContext context) async {
    ...
    double hightValue = currentButtonHeightPlace - x.dy;
