...
class OutgoingVideoCallScreen extends StatefulWidget {
  ...
  const OutgoingVideoCallScreen({Key? key, this.name, this.id, this.image})
      : super(key: key);

  @override
  _OutgoingVideoCallScreenState createState() =>
...
class _OutgoingVideoCallScreenState extends State<OutgoingVideoCallScreen> {
  ...
  _leaveChannel() async {
    ...
    databaseReference
        .child("Calls")
        .child(firebaseNode)
        .update({'status': 'call_ended_by_caller'});
    await rtcEngine.leaveChannel();
    Get.back();
  }
  ...
  _joinChannel() async {
    databaseReference
      ...
      if (snapShot.value != null) {
        Fluttertoast.showToast(msg: "User is busy at the moment");
        Get.back();
      } else {
        await rtcEngine
                    ...
                    Align(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 10),
                          textUtils.bold20(widget.name, AppColors.whiteColor,
                              TextAlign.center),
                          SizedBox(height: 10),
                          textUtils.bold20('Ringing', AppColors.whiteColor,
                              TextAlign.center),
                        ],
                      ),
                    ),
